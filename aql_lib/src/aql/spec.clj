...
(s/def ::attribute (s/tuple string? string? string?))
(s/def ::attributes (s/coll-of ::attribute :kind vector?))
(s/def ::reference (s/tuple string? string? string?))
(s/def ::references (s/coll-of ::reference :kind vector?))
(s/def ::alias (s/+ (s/cat :a string? :r string?)))
(s/def ::relation #{::equal ::lt})
(s/def ::expression (s/coll-of (s/or :term string? :sub ::expression)
                               :kind vector?))
